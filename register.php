...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($errors === []) {
        ...
        if ($result['ok'] && is_array($result['user'])) {
            flash('registered', 'Account created successfully. You can sign in now.');
            redirect('/');
        }
    ...
    $safeOld = $data;
    unset($safeOld['password'], $safeOld['password_confirm']);
...
function reg_value(array $old, string $field): string { return isset($old[$field]) && is_string($old[$field]) ? $old[$field] : ''; }
function reg_error(array $errors, string $field): ?string { return isset($errors[$field]) && is_string($errors[$field]) ? $errors[$field] : null; }
?>
<!doctype html>
<html lang="en">
<head>
<meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">
<meta name="description" content="Create a secure AccountFoundry demo account.">
<meta name="color-scheme" content="light dark">
<title>AccountFoundry — Register</title>
...
<p class="eyebrow">Account creation / 02</p>
<h1>Registration is part of the security boundary.</h1>
<p>Validation runs before database writes, username/email uniqueness is enforced again in storage, and passwords are hashed before they ever reach a table.</p>
</section>

<section class="register-card">
<h2>Create account</h2>
<p class="form-intro">Phone is optional. Passwords must be at least 12 characters.</p>
...
<div class="field"><label for="phone">Phone <span>optional</span></label><input id="phone" name="phone" type="tel" autocomplete="tel" maxlength="24" value="<?= e(reg_value($old,'phone')) ?>" <?= reg_error($errors,'phone') ? 'aria-invalid="true" aria-describedby="phone-error"' : '' ?>><?php if($e=reg_error($errors,'phone')):?><p id="phone-error" class="field-error"><?=e($e)?></p><?php endif;?></div>
...
<p class="switch-link">Already registered? <a href="/">Sign in</a></p>
</section>
</main>
</body>
</html>